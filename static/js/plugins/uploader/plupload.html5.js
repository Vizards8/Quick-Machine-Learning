        ...
        init: function(p, q) {
            ...
            function(v) {
                ...
                if (E) {
                    ...
                    w = v.features.triggerDialog ? E: F;
                    if (x) {
                        j.addEvent(w, "mouseover",
                        function() {
                            j.addClass(E, x)
                        },
                        v.id);
                        j.addEvent(w, "mouseout",
                        function() {
                            j.removeClass(E, x)
                        },
                        v.id)
            ...
            function(r, t) {
                ...
                function v(z) {
                    var C = 0,
                    B = 0,
                    y = ("FileReader" in h) ? new FileReader: null;
                    function A() {
                        ...
                        function F(V) {
                            ...
                            if (r.settings.multipart && n.multipart) {
                                ...
                                T.open("post", D, true);
                                j.each(r.settings.headers,
                                function(Y, X) {
                                    T.setRequestHeader(X, Y)
                                });
                                if (typeof(V) !== "string" && !!h.FormData) {
                                    N = new FormData();
                                    j.each(j.extend(K, r.settings.multipart_params),
                                    function(Y, X) {
                                        N.append(X, Y)
                                    });
                ...
                x = c[t.id];
                if (n.jpgresize && r.settings.resize && /\.(txt|jpg|jpeg)$/i.test(t.name)) {
                    d.call(r, t, r.settings.resize, /\.txt$/i.test(t.name) ? "txt/csv": "txt/csv",
                    function(y) {
                        if (y.success) {
